...
// memoriadocavaquinhobrasileiro@gmail.com. Gere a sua em https://web3forms.com
// e cole aqui.
const WEB3FORMS_ACCESS_KEY = 'ff860148-1bb8-4eff-9f55-148eaf4b9ede'
...
export default function Contato() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      from_name: 'Site Memória do Cavaquinho',
      name: fd.get('name'),
      email: fd.get('email'),
      subject: `[Contato Site] ${fd.get('subject') || 'Nova mensagem'}`,
          ...
          <div className={styles.contatoGrid}>
            ...
            <form className={styles.form} onSubmit={handleSubmit} aria-label="Formulário de contato">
              <div className={styles.formField}>
                ...
                <input className={styles.formInput} type="text" id="contact-name" name="name" placeholder="Seu nome completo" required />
              </div>
              <div className={styles.formField}>
                <label className={styles.formLabel} htmlFor="contact-email">E-mail</label>
                <input className={styles.formInput} type="email" id="contact-email" name="email" placeholder="seu@email.com.br" required />
              </div>
              <div className={styles.formField}>
                <label className={styles.formLabel} htmlFor="contact-subject">Assunto</label>
                <input className={styles.formInput} type="text" id="contact-subject" name="subject" placeholder="Assunto da mensagem" required />
              </div>
              <div className={styles.formField}>
                <label className={styles.formLabel} htmlFor="contact-message">Mensagem</label>
                <textarea className={styles.formTextarea} id="contact-message" name="message" placeholder="Escreva sua mensagem..." required />
              </div>
              <button
                type="submit"
              ...
              {error && (
                <p role="alert" style={{ color: '#b91c1c', fontSize: '0.85rem', margin: 0 }}>{error}</p>
              )}
            </form>
          </div>
        </div>
